...
export default function WizardStep1({ data, onChange, onNext }: WizardStep1Props) {
    ...
    //
    // Shake dua target:
    //   1. .s1-scope .input-error  → field yang gagal (timeout 50ms agar React
    //      sempat apply class sebelum animejs select)
    //   2. .s1-submit              → tombol Lanjut
    ...
    return (
        <div className="s1-scope">
            ...
            <div className="space-y-5 sm:space-y-6">
                ...
                <div className="s1-field opacity-0">
                    <label className="label">
                        NISN (10 Angka) <span className="text-rose-500">*</span>
                    </label>
                    <input
                        type="text"
                        name="nisn"
                    ...
                    {errors.nisn && (
                        <span className="error-text">{errors.nisn}</span>
                    )}
                </div>
                ...
                <div className="s1-field opacity-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input
                            ...
                            onChange={e =>
                                set('place_of_birth', e.target.value.replace(/[^a-zA-Z\s]/g, ''))
                            }
                        ...
                        {errors.place_of_birth && (
                            <span className="error-text">{errors.place_of_birth}</span>
                        )}
                    </div>
                    <div>
                        <label className="label">
                ...
                <div className="s1-field opacity-0 flex flex-col sm:flex-row items-center justify-between gap-3 pt-5 border-t border-slate-200">
                    <p className="text-[11px] text-slate-400 text-center sm:text-left">
                        Langkah 1 dari 3 &bull; Mohon diisi dengan teliti.
                    </p>
                    <button
                        type="button"
                        onClick={handleNext}
                        className="s1-submit w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full bg-navy text-gold px-7 py-3.5 text-sm font-semibold shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95 transition-all"
                    >
                        Lanjut ke Langkah 2
