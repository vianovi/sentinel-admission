...
/** Normalisasi nomor HP → format 08xxx, max 14 digit */
const formatHP = (val: string): string => {
...
export default function WizardStep2({
    ...
    const validate = (): boolean => {
        ...
        if (Object.keys(e).length > 0) {
            ...
            // Shake fields setelah React apply class .input-error
            setTimeout(() => {
                animate('.s2-scope .input-error', {
            ...
            <div className="s2-field opacity-0 mb-5 sm:mb-8 border-b border-slate-200 pb-4 flex justify-between items-start gap-3">
                <div>
                    ...
                    <p className="text-xs sm:text-sm text-slate-500 mt-1">
                        Pastikan alamat dan kontak aktif agar panitia mudah menghubungi.
                    </p>
                </div>
                {candidateName && (
            ...
            <div className="space-y-5 sm:space-y-6">
                ...
                <div className="s2-field opacity-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        {errors.whatsapp_number && (
                            <span className="error-text">{errors.whatsapp_number}</span>
                        )}
                    </div>
                    <div>
                        <label className="label">
                ...
                <div className="s2-field opacity-0">
                    ...
                    <input
                        ...
                        onChange={e => set('phone_number', formatHP(e.target.value))}
                        placeholder="Contoh: 081234567890"
                        className="input-field"
                    />
                    <p className="hint">No. HP orang tua / wali selain WhatsApp. Boleh dikosongkan.</p>
                </div>
                    ...
                    <div className="space-y-3 sm:space-y-4">
                        ...
                        <div>
                            <label className="label text-xs">
                                Jalan / Dusun / Perumahan <span className="text-rose-500">*</span>
                            </label>
                            <input
                                type="text"
                                name="addr_jalan"
                            ...
                            />
                            {errors.addr_jalan && (
                                <span className="error-text">{errors.addr_jalan}</span>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    value={data.addr_rt}
                                    onChange={e =>
                                        set('addr_rt', e.target.value.replace(/\D/g, '').slice(0, 3))
                                    }
                                    placeholder="005"
                                    className={`input-field ${errors.addr_rt ? 'input-error' : ''}`}
                                />
                                {errors.addr_rt && (
                                    <span className="error-text">{errors.addr_rt}</span>
                                )}
                            </div>
                            <div>
                                <label className="label text-xs">
                                ...
                                <input
                                    ...
                                    value={data.addr_rw}
                                    onChange={e =>
                                        set('addr_rw', e.target.value.replace(/\D/g, '').slice(0, 3))
                                    }
                                    placeholder="007"
                                    className={`input-field ${errors.addr_rw ? 'input-error' : ''}`}
                                />
                                {errors.addr_rw && (
                                    <span className="error-text">{errors.addr_rw}</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                />
                                {errors.addr_desa && (
                                    <span className="error-text">{errors.addr_desa}</span>
                                )}
                            </div>
                            <div>
                                <label className="label text-xs">
                                ...
                                />
                                {errors.addr_kec && (
                                    <span className="error-text">{errors.addr_kec}</span>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                ...
                                />
                                {errors.addr_kab && (
                                    <span className="error-text">{errors.addr_kab}</span>
                                )}
                            </div>
                            <div>
                                <label className="label text-xs">
                                ...
                                />
                                {errors.addr_prov && (
                                    <span className="error-text">{errors.addr_prov}</span>
                                )}
                            </div>
                        </div>
                ...
                <div className="s2-field opacity-0 flex flex-col sm:flex-row items-center justify-between gap-3 pt-5 border-t border-slate-200">
                    ...
                    <button
                        type="button"
                        onClick={handleNext}
                        className="s2-submit w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full bg-gold text-slate-900 px-7 py-3.5 text-sm font-semibold shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95 transition-all"
                    >
                        Simpan &amp; Lanjut
